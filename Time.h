...
public:
	...
	bool checkForConflict(Time toCheck)   
	{
		...
		if (checkTimeOverlap(this->startY, toCheck.startY, this->endY, toCheck.endY ) )// Do they happen in the same year(s)
		{
			...
			{
				if (checkDayOfMonthOverlap(toCheck))// Do they try to meet at the same within the month
				{
					if (checkDayOverlap(this->courseWeek, toCheck.courseWeek))// Do they ever meet on the same day in the week (MWF, TR)
					{
